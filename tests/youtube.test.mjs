...
test('YouTube bağlantıları kanonikleştirilir; dış adresler ve komutlar reddedilir', () => {
  ...
  assert.match(youtubeError(new Error('Sign in to confirm you are not a bot')), /oturum/);
});

test('YouTube kuyruğu ve gerçek FFmpeg ile MP3/MP4 hazırlama', { timeout: 90000 }, async t => {
  ...
  const metadata = { title: 'Test Listesi / Yol', entries: [
    { id: 'aaaaaaaaaaa', title: 'Şarkı <test>', uploader: 'Türkçe sanatçı', duration: 2 },
    { id: 'bbbbbbbbbbb', title: 'Video', duration: 2 },
    { id: 'ccccccccccc', title: '[Private video]', availability: 'private' },
    { id: 'ddddddddddd', title: 'Hatalı dosya', duration: 2 },
    { id: 'eeeeeeeeeee', title: 'İptal testi', duration: 2 },
  ] };
  ...
  const runner = async (command, args, options = {}) => {
    ...
    options.onLine('RW_META:"Şarkı <test>"\t"Türkçe sanatçı"\t2');
  ...
  await t.test('Liste seçimi, erişilemeyen video ve kopya denetimi', async () => {
    ...
    assert.equal(info.format.tags.title, 'Şarkı <test>'); assert.equal(info.format.tags.artist, 'Türkçe sanatçı');
  ...
  await t.test('WebM/VP9/Opus gerçek H.264/AAC MP4 olur', async () => {
  ...
  await t.test('Bir hata diğer videoları durdurmaz; iptal ve devam çalışır', async () => {
  ...
  await t.test('Oturum korunur; listeden kaldırmak MP3/MP4 silmez', async () => {
