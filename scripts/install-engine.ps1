...
if ((Test-Path -LiteralPath (Join-Path $engineDir 'ffmpeg.exe')) -and (Test-Path -LiteralPath (Join-Path $engineDir 'ffprobe.exe'))) {
    Write-Output 'FFmpeg motoru hazir.'
    exit 0
}
...
Write-Output 'FFmpeg Windows motoru indiriliyor (yaklasik 110 MB)...'
& curl.exe --fail --location --silent --show-error --connect-timeout 20 --max-time 60 --output $checksumFile ($baseUrl + '.sha256')
if ($LASTEXITCODE -ne 0) { throw 'FFmpeg dogrulama bilgisi indirilemedi.' }
$expected = ((Get-Content -LiteralPath $checksumFile -Raw).Trim() -split '\s+')[0]
if ($expected -notmatch '^[a-fA-F0-9]{64}$') { throw 'Gecersiz SHA256 bilgisi.' }
$downloadNeeded = $true
...
if ($downloadNeeded) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'FFmpeg indirilemedi. Yeniden baslattiginizda indirme kaldigi yerden devam eder.' }
}
$actual = (Get-FileHash -LiteralPath $archive -Algorithm SHA256).Hash
if ($expected -notmatch '^[a-fA-F0-9]{64}$' -or $actual -ne $expected) { throw 'FFmpeg SHA256 dogrulamasi basarisiz. Kurulum durduruldu.' }
Write-Output 'Indirme dogrulandi. Motor aciliyor...'
...
if (-not $binary) { throw 'Arsivde FFmpeg bulunamadi.' }
...
Write-Output 'FFmpeg ve FFprobe hazir.'
