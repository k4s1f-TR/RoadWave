...
export function executableAvailable(command, { exists = fs.existsSync, spawnSyncImpl = spawnSync } = {}) {
  ...
  const versionArgs = ['ffmpeg', 'ffprobe'].includes(executable) ? ['-version'] : ['--version'];
  const result = spawnSyncImpl(command, versionArgs, { windowsHide: true, stdio: 'ignore', timeout: 10000 });
  return !result.error && result.status === 0;
}

export function ffmpegEncoderAvailable(command, encoder, { spawnSyncImpl = spawnSync } = {}) {
  if (!command || !/^[a-z0-9_]+$/i.test(encoder || '')) return false;
  const result = spawnSyncImpl(command, ['-hide_banner', '-encoders'], {
    windowsHide: true,
    encoding: 'utf8',
    timeout: 10000,
  ...
  return output.split(/\r?\n/).some(line => new RegExp(`^\\s*[A-Z.]{6}\\s+${encoder}\\s`, 'i').test(line));
}
