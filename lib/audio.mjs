...
export function conversionArgs(input, output, info, settings, withCover = true, externalCover = null) {
  ...
  if (isMp3) {
    ...
    const encoder = settings.vorbisEncoder === 'vorbis' ? 'vorbis' : 'libvorbis';
    args.push('-c:a', encoder, '-q:a', String(quality));
    if (encoder === 'vorbis') args.push('-strict', 'experimental');
    args.push('-ar', '44100', '-ac', '2', '-f', 'ogg');
